...
import { RawajFeatureSettings, RawajFeatureSettingsRequest } from '../../models/settings/rawaj-feature-settings.model';
...
const SAFE_DEFAULTS: RawajFeatureSettings = {
  id: 0,
  storeId: 0,
...
export class RawajFeatureSettingsService {
  private readonly http = inject(HttpClient);
  private readonly authService = inject(AuthService);
  private readonly apiUrl = `${environment.apiUrl}/settings/rawaj-features`;
  ...
  readonly flags = signal<RawajFeatureSettings>(SAFE_DEFAULTS);

  private getStoreId(): number {
    return this.authService.getStoreId() || 1;
  }

  getSettings(): Observable<RawajFeatureSettings> {
    const storeId = this.getStoreId();

    return this.http.get<ApiResponse<RawajFeatureSettings>>(this.apiUrl, {
      ...
      catchError(this.handleError<RawajFeatureSettings>('getSettings', this.flags()))
    );
  }

  updateSettings(request: RawajFeatureSettingsRequest): Observable<RawajFeatureSettings> {
    const storeId = this.getStoreId();

    return this.http.put<ApiResponse<RawajFeatureSettings>>(this.apiUrl, request, {
      params: new HttpParams().set('storeId', storeId)
    }).pipe(
      map(response => response.data),
      tap(settings => this.flags.set(settings)),
      catchError(this.handleError<RawajFeatureSettings>('updateSettings'))
    );
  }
