...
# 🛒 Rawaj Supermarket — Web Frontend
...
**Rawaj Supermarket** is a retail management system originally forked from [SmartPharma](https://github.com/amer-rouby/smartpharma-frontend), reusing its Angular architecture, design system, and feature set (dashboard, POS, inventory, purchasing, payments, reporting, i18n) as the base for a **supermarket** business instead of a pharmacy. A dedicated set of "Rawaj AI" features (below) has since been built on top of that base specifically for supermarket operations.
...
Some copy/validation still reflects the pharmacy origin in places that were never business-critical to rewrite (e.g. batch/expiry framing originally tuned for medicine). Core supermarket workflows (POS, inventory, purchasing, the Rawaj AI feature set above) are fully adapted.
